...
struct __anon_type_16 {
  int __count;
  union __anon_type_15 __value;
} ;
typedef struct __anon_type_16 __mbstate_t;
struct __anon_type_17 {
  __off_t __pos;
  __mbstate_t __state;
} ;
typedef struct __anon_type_17 _G_fpos_t;
struct __anon_type_18 {
  __off64_t __pos;
  __mbstate_t __state;
} ;
typedef struct __anon_type_18 _G_fpos64_t;
...
typedef int wchar_t;
struct __anon_type_19 {
  int quot;
  int rem;
} ;
typedef struct __anon_type_19 div_t;
struct __anon_type_20 {
  long int quot;
  long int rem;
} ;
typedef struct __anon_type_20 ldiv_t;
struct __anon_type_21 {
  long long int quot;
  long long int rem;
} ;
typedef struct __anon_type_21 lldiv_t;
...
int main_0(int argc, char **argv) {
  ...
  __CPAchecker_TMP_1 = strcmp(argv[optind], "-");
  __CPROVER_assume(!(__CPAchecker_TMP_1 == 0));
  infd = open(argv[optind], 0);
  __CPROVER_assume(!(infd == -1));
  int __CPAchecker_TMP_5;
  __CPAchecker_TMP_5 = fstat(infd, &st);
  __CPROVER_assume(__CPAchecker_TMP_5 == 0);
  data = mmap((void *)0, st.st_size, 1, 1, infd, 0);
  __CPROVER_assume(!(data == ((void *)-1)));
  int __CPAchecker_TMP_6;
  __CPAchecker_TMP_6 = strcmp(argv[optind + 1], "-");
  __CPROVER_assume(!(__CPAchecker_TMP_6 == 0));
  outfd = open(argv[optind + 1], 577, 420);
  __CPROVER_assume(!(outfd == -1));
  int __CPAchecker_TMP_10;
  __CPAchecker_TMP_10 = process_ihex_1(data, st.st_size);
}

int process_ihex_1(uint8_t *data, ssize_t size) {
  struct ihex_binrec *record;uint32_t offset = 0;uint32_t data32;uint8_t type;uint8_t crc = 0;uint8_t crcbyte = 0;int i;int j;int line = 1;int len;i = 0;
...
uint8_t  hex_2(const uint8_t *data, uint8_t *crc) {
  ...
  uint8_t val = (__CPAchecker_TMP_0 << 4) | __CPAchecker_TMP_1;*crc = (*crc) + val;return val;
}
